...
# trademark
trademark_columns = ["doc", "caseno", "registerno", "trademark_name", "trademark_design", "filing_date",
                     "censor", "priority_date", "sdate", "edate", "word_description", "mark_type", "memo", "wavpath", "servicemark"]
# rca(agent)
...
def sql_command2(cur, colnames, tablename, caseno_list):
    ...
    cur.execute("SELECT {} FROM {} WHERE {}".format(
        selected, tablename, conditions))
    df = pd.DataFrame(cur.fetchall(), columns=colnames).astype(str)
    df = df.groupby(df['caseno'], as_index=False).aggregate(
        lambda x: "/".join(x))
    return df
...
def fetchdata4(caseno_list):
    ...
    df_all = reduce(lambda left, right: pd.merge(left, right, how='left', on='caseno'), [
                    main_df, rca_df, rcc_df, rco_df, rcp_df, text_df]).astype(str)
    ...
    for i in df_all.to_dict(orient='records'):
        result = dict()
        result["Path"] = '/service/trademark/raw_register_data/' + \
            i['doc'] + "/" + i['filename']
...
def create_html(TYPE, SEARCH, TIME, resultPath_list):
    dirr = time.strftime(
        "/home/metadragons/backend/SearchHtmlResults/{}/%Y-%m-%d").format(TYPE)
    # print("dirr: ", dirr)
...
def load_images(TYPE, SEARCH, TIME, caseno_list):
    ...
    for r in result_list:
        try:
            ...
            pil_img.save(byte_arr, format='PNG', quality=50)
            ...
            pass
    html_str = create_html(TYPE, SEARCH, TIME, [
                           r["Path"] for r in result_list])
...
def function5():
    ...
    if searchQuery != "":
        print(request.remote_addr, datetime.datetime.now(),
              "TextSearch:", searchQuery, file=sys.stdout)
        ...
        EndTime3 = time.time()
        print("Time Spent(Text_Model): {}s".format(
            EndTime3 - startTime3), file=sys.stdout)
    startTime4 = time.time()
    base64Image_list1, metadata_list1, base64Image_list2, metadata_list2 = [], [], [], []
    if photo != "null":
        base64Image_list1, metadata_list1 = load_images(
            "Image", filePath, request.form['name'], caseno_list1)
    if searchQuery != "":
        base64Image_list2, metadata_list2 = load_images(
            "Text", searchQuery, request.form['name'], caseno_list2)
    EndTime4 = time.time()
    print("Time Spent(load_images): {}s".format(
        EndTime4 - startTime4), file=sys.stdout)
    EndTime = time.time()
    print("Time Spent(function5): {}s".format(
...
def function6():
    ...
    for path in path_list:
        ...
        base64Image_list.append(encoded_img)
    EndTime = time.time()
    print(request.remote_addr, "Time Spent(function6): {}s".format(
        EndTime - startTime), file=sys.stdout)
    return jsonify({
